...
public final class AvaritiaWeaponUtil {
     ...
     * 纯读取实现：不写入任何组件（ItemUtils#getOrCreateChildTag 内部会 set CUSTOM_DATA，
     * 且 CustomData.of 会拷贝 NBT，读后修改的写法是无效的——见 {@link #setMode}）。
     */
    public static boolean isModeActive(ItemStack stack, String mode) {
        ...
        return root.contains("mode", CompoundTag.TAG_COMPOUND) && root.getCompound("mode").getBoolean(mode);
    }

    /**
     * 设置无尽武器模式并正确写回物品组件。
     *
     * 1.21.1 起 ItemStack#getOrCreateTagElement 被移除，且 Re-Avaritia 的
     * ItemUtils#getOrCreateChildTag 在返回前用 CustomData.of（内部拷贝 NBT）写回组件，
     * 因此“先取子标签、后 putBoolean”的写法改动的是组件外的一份副本，永远不会生效
     * （武器模式切换按钮失效的根因）。这里按 Re-Avaritia ISwitchable#switchMode 的
     * 正确顺序实现：先改后写。
     */
    public static void setMode(ItemStack stack, String mode, boolean value) {
        CompoundTag root = ItemUtils.getOrCreateTag(stack);
        CompoundTag modeTag = root.contains("mode", CompoundTag.TAG_COMPOUND)
                ? root.getCompound("mode")
                : new CompoundTag();
        modeTag.putBoolean(mode, value);
    ...
    public static void applyForcedMode(ItemStack stack) {
        if (stack.getItem() instanceof InfinitySwordItem) {
            ...
            if (isModeActive(stack, "infinity_sword_kill") != forced) {
                setMode(stack, "infinity_sword_kill", forced);
            }
        }
    }
