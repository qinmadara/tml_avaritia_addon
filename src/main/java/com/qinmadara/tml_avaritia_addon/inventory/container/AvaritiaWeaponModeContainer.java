 ...
 * - TYPE 用 IForgeMenuType.create 从网络缓冲读取 entityId（TLM 齿轮按钮打开配置界面时
 *   通过 NetworkHooks.openScreen 写入 int entityId，DefaultMaidTaskConfigContainer 已验证此格式）；
 * - 客户端由 AvaritiaWeaponModeGui 渲染（标题 + 模式切换按钮）。
 */
public class AvaritiaWeaponModeContainer extends TaskConfigContainer {
    public static final MenuType<AvaritiaWeaponModeContainer> TYPE = IForgeMenuType.create(
