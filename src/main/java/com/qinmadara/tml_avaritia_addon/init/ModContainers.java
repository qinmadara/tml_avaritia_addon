 ...
 * 本附属的容器（MenuType）注册。
 * <p>
 * 对齐 TLM InitContainer 的 DeferredRegister 模式（ForgeRegistries.MENU_TYPES）。
 * 需在主类构造器里调用 ModContainers.CONTAINER_TYPES.register(modEventBus) 挂到 mod 事件总线，
 * 客户端再用 MenuScreens.register 绑定 GUI（见 ModClientInit）。
 */
public final class ModContainers {
    public static final DeferredRegister<MenuType<?>> CONTAINER_TYPES = DeferredRegister.create(
            ForgeRegistries.MENU_TYPES, tml_avaritia_addon.MODID);

    /** 无尽弓/弩"武器攻击模式切换"任务设置界面容器 */
    public static final RegistryObject<MenuType<AvaritiaWeaponModeContainer>> AVARITIA_WEAPON_MODE = CONTAINER_TYPES.register(
